...
$SELECT = "SELECT event_name,event_organiser,event_date,entry_id,entry_name,entry_regno,entry_position FROM events INNER JOIN " . $event_id . " ON events.event_id = " . $event_id . ".event_id WHERE entry_position!=0 AND entry_college='VIT' AND approval_status=1 AND generate_status=0";
...
// generate certificate for all internal winners
while ($row = mysqli_fetch_array($query)) {
    ...
    /* ----------------------VERIFICATION LINK---------------------------------------- */
    // save image as jpeg to event internal_winners folder
