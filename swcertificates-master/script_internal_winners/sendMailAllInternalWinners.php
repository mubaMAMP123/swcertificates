...
$SELECT = "SELECT event_name,event_organiser,event_date,entry_id,entry_name,entry_regno,entry_email,entry_position,certificate_number,verification_link FROM events INNER JOIN " . $event_id . " ON events.event_id = " . $event_id . ".event_id WHERE entry_position!=0 AND entry_college = 'VIT' AND generate_status=1 AND mail_status!=1";
$query = mysqli_query($conn, $SELECT);
if ($query->num_rows == 0) {
  $conn->close();
  $qstring = '?eid=' . $event_id . '&status=514';
...
//send mail to internal winners whose send mail status not equal to 1
while ($row = mysqli_fetch_array($query)) {
  ...
  $internal_winner_id = $row['entry_id'];
  ...
  $certificate_path = "../certificates/" . $event_id . "/internal_winners/" . $entry_regno . ".jpg";
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    ...
    <div style="background-color: #21295d; color: #fff; padding: 10px; line-height: 1.5; text-align: center;">
      ...
      <br /><br />
      <a href="http://localhost/swcertificates/certificates/$event_id/internal_winners/$entry_regno.jpg" style="text-decoration:none;">
...
EOT;
  ...
  if ($mail->send()) {
    $UPDATE = "UPDATE " . $event_id . " SET mail_status=1, mailed_by='" . $mailed_by . "', mailed_at='" . $mailed_at . "' WHERE entry_id = '" . $internal_winner_id . "'";
    $conn->query($UPDATE);
  } else {
    $UPDATE = "UPDATE " . $event_id . " SET mail_status=2 WHERE entry_id = '" . $internal_winner_id . "'";
    $conn->query($UPDATE);
  }
  $mail->smtpClose();
}
...
$qstring = '?eid=' . $event_id . '&status=515';
//redirect to respective page with status
if ($account_type == "u") :
  header("Location: ../event" . $qstring);
  exit;
