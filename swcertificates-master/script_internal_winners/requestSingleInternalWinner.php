...
$UPDATE = "UPDATE " . $event_id . " SET request_status=1, requested_by='" . $requested_by . "', requested_at='" . $requested_at . "' WHERE entry_id = '" . $internal_winner_id . "' AND request_status=0";
$result_update = $conn->query($UPDATE);

if ($result_update === TRUE) {
    $qstring = '?eid=' . $event_id . '&status=13';
} else {
    $qstring = '?eid=' . $event_id . '&status=14';
}
