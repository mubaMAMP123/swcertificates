...
// get internal winner id, event id and approval status
...
// update approval status of the internal winner
$UPDATE = "UPDATE " . $event_id . " SET approval_status='" . $status . "', approved_by='" . $approved_by . "', approved_at='" . $approved_at . "' WHERE entry_id = '" . $internal_winner_id . "' AND request_status=1 AND approval_status=0";
...
if ($result_update === TRUE) {
    if ($status == 1) {
        ...
        $qstring = '?eid=' . $event_id . '&status=12';
    }
} else {
    $qstring = '?eid=' . $event_id . '&status=11';
}
