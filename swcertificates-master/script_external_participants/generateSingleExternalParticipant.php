...
// get external participant id and event id
...
$SELECT = "SELECT event_name,event_organiser,event_date,entry_name,entry_college FROM events INNER JOIN " . $event_id . " ON events.event_id = " . $event_id . ".event_id WHERE entry_id='" . $external_participant_id . "' AND approval_status=1 AND generate_status=0";
...
$certificate_number = substr($event_id, 2, 7) . sprintf('%03s', $external_participant_id);
...
// template
$image = imagecreatefromjpeg("../templates/Participant_External.jpg");
global $image_width;
...
/* ----------------------VERIFICATION LINK---------------------------------------- */
// save image as jpeg to event external_participant folder
imagejpeg($image, "../certificates/$event_id/external_participants/$certificate_number.jpg");
imagedestroy($image);
...
if ($certificate_exists === TRUE) :
    ...
    $UPDATE = "UPDATE " . $event_id . " SET generate_status=1, generated_by='" . $generated_by . "', generated_at='" . $generated_at . "', certificate_number = '" . $certificate_number . "', verification_link = '" . $link . "' WHERE entry_id = '" . $external_participant_id . "'";
    $conn->query($UPDATE);
    $qstring = '?eid=' . $event_id . '&status=39';
else :
    $qstring = '?eid=' . $event_id . '&status=40';
endif;
