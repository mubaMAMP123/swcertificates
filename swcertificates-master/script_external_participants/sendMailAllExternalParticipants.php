...
//select event and external participant details
$SELECT = "SELECT event_name,event_organiser,event_date,entry_id,entry_name,entry_email,entry_college,certificate_number,verification_link FROM events INNER JOIN " . $event_id . " ON events.event_id = " . $event_id . ".event_id WHERE entry_position=0 AND entry_college != 'VIT' AND generate_status=1 AND mail_status!=1";
$query = mysqli_query($conn, $SELECT);
if ($query->num_rows == 0) :
  $conn->close();
  $qstring = '?eid=' . $event_id . '&status=528';
...
//send mail to external participants whose send mail status not equal to 1
while ($row = mysqli_fetch_array($query)) :
  ...
  $external_participant_id = $row['entry_id'];
  ...
  $certificate_path = "../certificates/" . $event_id . "/external_participants/" . $certificate_number . ".jpg";
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    ...
    <div style="background-color: #21295d; color: #fff; padding: 10px; line-height: 1.5; text-align: center;">
      ...
      <br /><br />
      <a href="http://localhost/swcertificates/certificates/$event_id/external_participants/$certificate_number.jpg" style="text-decoration:none;">
...
EOT;
  ...
  if ($mail->send()) :
    $UPDATE = "UPDATE " . $event_id . " SET mail_status=1, mailed_by='" . $mailed_by . "', mailed_at='" . $mailed_at . "' WHERE entry_id = '" . $external_participant_id . "'";
    $conn->query($UPDATE);
  else :
    $UPDATE = "UPDATE " . $event_id . " SET mail_status=2 WHERE entry_id = '" . $external_participant_id . "'";
    $conn->query($UPDATE);
  endif;
  $mail->smtpClose();
endwhile;
...
$qstring = '?eid=' . $event_id . '&status=529';
