...
// get internal participant id and event id
...
$SELECT = "SELECT event_name,event_organiser,event_date,entry_name,entry_regno FROM events INNER JOIN " . $event_id . " ON events.event_id = " . $event_id . ".event_id WHERE entry_id='" . $internal_participant_id . "' AND approval_status=1 AND generate_status=0";
...
$certificate_number = substr($event_id, 2, 7) . sprintf('%03s', $internal_participant_id);
...
/* ----------------------VERIFICATION LINK---------------------------------------- */
// save image as jpeg to event internal_participant folder
...
if ($certificate_exists === TRUE) :
    ...
    $UPDATE = "UPDATE " . $event_id . " SET generate_status=1, generated_by='" . $generated_by . "', generated_at='" . $generated_at . "', certificate_number = '" . $certificate_number . "', verification_link = '" . $link . "' WHERE entry_id = '" . $internal_participant_id . "'";
    $conn->query($UPDATE);
    $qstring = '?eid=' . $event_id . '&status=49';
else :
    $qstring = '?eid=' . $event_id . '&status=50';
endif;
