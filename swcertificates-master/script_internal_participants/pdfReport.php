...
$SELECT_INTERNAL_PARTICIPANT = "SELECT * FROM " . $event_id . " WHERE entry_position = 0 AND entry_college = 'VIT' AND generate_status=1";
$result = mysqli_query($conn, $SELECT_INTERNAL_PARTICIPANT);
$emparray = array();
...
$html = <<<EOD
    ...
    <title>$event_name Internal Participants Report</title>
    <style>
      body {
        margin: 0;
        padding: 0;
  ...
  <body>
    ...
    <div class='sub-heading'>
      <h2>Event Internal Participants Report</h2>
    </div>
    <div class='content'>
      <h3>Event Details</h3>
      <table id='event'>
...
foreach ($data_decoded as $entry) {
  $html .= <<<EOD
          <tr>
            ...
            <td><a href='http://localhost/swcertificates/certificates/$event_id/internal_participants/$entry->entry_regno.jpg'>View</a></td>
          </tr>
          EOD;
  $count++;
}
...
$dompdf->stream("$event_name Internal Participants' Report", array("Attachment" => 1));
