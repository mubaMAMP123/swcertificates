...
$SELECT = "SELECT event_name,event_organiser,event_date,entry_id,entry_name,entry_regno,entry_email,certificate_number,verification_link FROM events INNER JOIN " . $event_id . " ON events.event_id = " . $event_id . ".event_id WHERE entry_position=0 AND entry_college = 'VIT' AND generate_status=1 AND mail_status!=1";
$query = mysqli_query($conn, $SELECT);
if ($query->num_rows == 0) {
  $conn->close();
  $qstring = '?eid=' . $event_id . '&status=536';
...
//send mail to internal participants whose send mail status not equal to 1
while ($row = mysqli_fetch_array($query)) {
  ...
  $internal_participant_id = $row['entry_id'];
...
$qstring = '?eid=' . $event_id . '&status=537';
//redirect to respective page with status
if ($account_type == "u") :
  header("Location: ../event" . $qstring);
  exit;
