...
$UPDATE = "UPDATE guests SET request_status=1, requested_by='" . $requested_by . "', requested_at='" . $requested_at . "' WHERE event_id = '" . $event_id . "' AND request_status=0";
$result_update = $conn->query($UPDATE);

if ($result_update === TRUE) {
    $qstring = '?eid=' . $event_id . '&status=7';
} else {
    $qstring = '?eid=' . $event_id . '&status=8';
}
