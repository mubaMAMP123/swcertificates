...
$SELECT = "SELECT event_name,event_organiser,event_date,entry_name,entry_college,entry_position FROM events INNER JOIN " . $event_id . " ON events.event_id = " . $event_id . ".event_id WHERE entry_id='" . $external_winner_id . "' AND approval_status=1 AND generate_status=0";
...
$certificate_number = substr($event_id, 2, 7) . sprintf('%03s', $external_winner_id);
...
// save image as jpeg to event external_winner folder
...
if ($certificate_exists === TRUE) :
    ...
    $UPDATE = "UPDATE " . $event_id . " SET generate_status=1, generated_by='" . $generated_by . "', generated_at='" . $generated_at . "', certificate_number = '" . $certificate_number . "', verification_link = '" . $link . "' WHERE entry_id = '" . $external_winner_id . "'";
    $conn->query($UPDATE);
    $qstring = '?eid=' . $event_id . '&status=29';
else :
    $qstring = '?eid=' . $event_id . '&status=30';
endif;
