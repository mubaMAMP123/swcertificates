...
$SELECT_EXTERNAL_WINNER = "SELECT * FROM " . $event_id . " WHERE entry_position != 0 AND entry_college != 'VIT' AND generate_status=1";
$result = mysqli_query($conn, $SELECT_EXTERNAL_WINNER);
$emparray = array();
...
$html = <<<EOD
    ...
    <title>$event_name External Winners Report</title>
    <style>
      body {
        margin: 0;
        padding: 0;
  ...
  <body>
    ...
    <div class='sub-heading'>
      <h2>Event External Winners Report</h2>
    </div>
    <div class='content'>
      <h3>Event Details</h3>
      <table id='event'>
...
foreach ($data_decoded as $entry) {
  $html .= <<<EOD
          <tr>
            ...
            <td><a href='http://localhost/swcertificates/certificates/$event_id/external_winners/$entry->certificate_number.jpg'>View</a></td>
          </tr>
          EOD;
  $count++;
}
...
$dompdf->stream("$event_name External Winners' Report", array("Attachment" => 1));
